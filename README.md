...
### Q: 更新配置后，节点订阅和 MITM 证书丢了？
A: 先分清两种操作，后果完全不同：
- **「更新」现有远程配置**（配置文件页对当前配置执行更新）：Quantumult X 会保留你自行添加的节点订阅和已生成的 MITM 证书，日常更新用这个，不会丢；
- **重新「下载/导入」配置或更换订阅链接**：相当于整套替换配置文件。节点订阅（存在配置的 `server_remote` 段）和 MITM 证书（生成后写在配置的 `mitm` 段）都会丢失，需要补回：
  1. 订阅：风车 → 节点 → 订阅 → 重新粘贴机场订阅链接并更新；
  2. 证书：风车 → MitM → 生成证书 → 安装描述文件，再到 iOS「设置 → 通用 → 关于本机 → 证书信任设置」开启完全信任。

> 本项目配置刻意不携带节点订阅和证书（公开仓库防泄露），换链接或重新导入前，建议先导出一份当前配置做备份。
